        ...
        *b_mat_ptr = (int *)malloc(*m_ptr * *l_ptr * sizeof(int));
        ...
        for (int i = 0; i < (*n_ptr); i++) {
            for (int j = 0; j < (*m_ptr); j++) {
                fscanf(file, "%d", &(*a_mat_ptr)[i * (*m_ptr) + j]);
            }
        }

        for (int i = 0; i < (*m_ptr); i++) {
            for (int j = 0; j < (*l_ptr); j++) {
                fscanf(file, "%d", &(*b_mat_ptr)[i * (*l_ptr) + j]);
            }
        }
        }
        ...
        int *local_a_mat = (int *)malloc(local_elements_a * sizeof(int));
        MPI_Scatter(a_mat_ptr, local_elements_a, MPI_INT, *local_a_mat, local_elements_a, MPI_INT, 0, MPI_COMM_WORLD);

        // Scatter matrix B data to all processes
        // MPI_Scatter(B, local_elements_b, MPI_INT, *b_mat_ptr, local_elements_b, MPI_INT, 0, MPI_COMM_WORLD);
        MPI_Bcast(b_mat_ptr, local_elements_b, MPI_INT, 0, MPI_COMM_WORLD);
        // Now each process has its portion of matrix A and B
        fclose(file)
        int *local_a_mat = (int *)malloc((n / world_size) * m * sizeof(int));
...
void matrix_multiply(const int n, const int m, const int l,const int *a_mat, const int *b_mat)
{
  int rank, size;
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    if (rank == 0) {
        *result_mat = (int *)malloc(n * l * sizeof(int));
    ...
    for (int i = 0; i < local_n; i++) {
        for (int j = 0; j < l; j++) {
            ...
            for (int k = 0; k < m; k++) {
                local_result[i * l + j] += a_mat[i * m + k] * b_mat[k * l + j];
            }
        }
    }

    // Gather the results from all processes to process 0
    MPI_Gather(local_result, local_n * l, MPI_INT, result_mat, local_n * l, MPI_INT, 0, MPI_COMM_WORLD);
